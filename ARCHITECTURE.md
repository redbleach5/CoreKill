...
- **Reasoning Models** — поддержка DeepSeek-R1, QwQ с автопарсингом `<think>` блоков
- **Structured Output** — Pydantic валидация через `generate_structured()`
- **Удалённый Ollama** — подключение к GPU через Tailscale
