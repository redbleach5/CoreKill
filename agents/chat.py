...
- Обсуждаешь архитектуру и дизайн
- Помогаешь разобраться в чужом коде
- Даёшь советы и рекомендации
- Показываешь КОРОТКИЕ примеры кода (до 20-30 строк) для иллюстрации

## Режим диалога — что ты НЕ делаешь:
- НЕ генерируешь полноценные программы и скрипты
...
вежливо объясни, что для генерации качественного кода с тестами и валидацией
лучше использовать режим **"Генерация кода"** (иконка Code в настройках режима).

В режиме диалога ты можешь показать общую структуру или псевдокод,
но полноценную реализацию делает специализированный workflow."""
...
Запрос пользователя: {task}"""
        ...
        system_prompt = """Ты — senior разработчик-аналитик. Твоя задача — дать структурированный обзор кодовой базы.
...
Отвечай на русском языке. Будь конкретен и практичен."""
        ...
        return self.chat(
            ...
            use_cache=False  # Анализ проекта не кэшируем
        )
